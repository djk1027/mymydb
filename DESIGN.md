...
## v1 (완료)
- [x] 테스트 프로그램을 만들고 케이스를 여러개 작성할 것 (overflow, 대용량 select, 대용량 insert 등)
  → `tests/` (`make test`). overflow / 100k insert / 대용량·필터 select / 에러 경로 등
- [x] 벤치마크 테스트 프로그램을 실행시킬 수 있는 벤치마크 프로그램을 만들것
  → `bench/` (`make bench [N]`). insert 처리량, full scan, WHERE 선택도별 처리량 측정
- [x] 용도별로 디렉터리를 분리할 것 (parse, optimize, execute 같이)
  → `src/{parse,optimize,execute,storage,cli}/`
- [x] 메모리 누수 등 프로그램 안정성 테스트를 같이 포함
